...
def pulse_shaping(i_q_samples: np.ndarray, Ts: float, OSF: float,
                  filter_len: int = 49, alpha: float = 0.25) -> (np.ndarray, Any):
    """
    ...
    :return: filtered smooth vector after pulse shaping, the filter h_rrc
    """
...
def match_filter(signal: np.ndarray, h_rrc):
    return np.convolve(signal, h_rrc)


def depulse_shaping(signal: np.ndarray, Ts: float, OSF: float,
    ...
    y = scipy.signal.lfilter(h_rrc, np.ones(1), signal)
