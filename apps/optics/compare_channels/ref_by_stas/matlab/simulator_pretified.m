...
% import lib/FNFT-linux/matlab
% print current workdir
addpath(genpath('../../../../lib/FNFT-linux/matlab'));
addpath(genpath('apps/optics/ref_by_stas/matlab'));
plots_dir='plots';
...
save_plot(xi,psi_xi,'\xi','\psi(\xi)',plots_dir,'psi_xi.png');
...
for ii=1:Nbursts
    ...
    save_plot(xi,abs(uin(ii,:)),'\xi','|u(\xi)|',plots_dir,sprintf('uin_%d.png',ii));
end
disp('Done')
toc
%% INFT @Tx
...
for ii=1:Nbursts  %The outer loop is over bursts
    ...
    xi_padded = linspace(XI(1), XI(end), Nnft);
    save_plot(xi_padded,abs(bin_padded),'\xi','|b(\xi)|',plots_dir,sprintf('bin_%d.png',ii));
    qin=mex_fnft_nsev_inverse(bin_padded, XI, [], [], Nnft, tau, 1,'cstype_b_of_xi');
    % save_plot(t,abs(qin),'\tau','|q_{in}(\tau)|',plots_dir,sprintf('qin_%d.png',ii));
...
xlim([-1,1]*Tb*Nbursts)
saveas(fig,fullfile(plots_dir,'power.png'));
%% Forward NFT @RX
tic
