...
class Metrics:
    ...
    @staticmethod
    def calc_ber_for_folder(all_x_read, all_y_read, conf_read, verbose=True, tqdm=None):
        num_errors = 0
        ber_vec = []
        ...
        if tqdm is not None:
            rng = tqdm(rng,total=len(all_x_read), leave=False)
    ...
    @staticmethod
    def gen_ber_mu_from_folders(root_dir, sub_name_filter, verbose_level=0, _tqdm=tqdm, num_x_per_folder=None, is_matrixed_ber=False):
        ...
        for dirpath in _tqdm(glob(f'{root_dir}/{sub_name_filter}')):
            ...
            ber = sub_ber_vec if is_matrixed_ber else np.mean(sub_ber_vec)
