...
class Trainer:
    def __init__(self,
        ...
        self.mean, self.std = GeneralMethods.calc_statistics_for_dataset(
            train_dataset)
    ...
    def train(self, num_epochs: int, mini_batch_size: int = 500, _tqdm=tqdm):
        ...
        mini_batch_size = min(mini_batch_size, len(
    ...
    def save3(self, dir_path: str = 'saved_models', endings: str = ''):
        ...
        torch.save(self.model.state_dict(),
                   sub_dir_path + '/model_state_dict.pt')
    ...
    def load3(cls, dir_path: str = 'saved_models') -> 'Trainer':
        ...
        assert dir_path and os.path.exists(
            abs_path), f'cant find the following path:\n\t{abs_path}'
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
        trainer = torch.load(dir_path + '/trainer.pt', map_location=device)
        model_state_dict = torch.load(
            dir_path + '/model_state_dict.pt', map_location=device)
    ...
    def plot_loss_vec(self):
        Visualizer.plot_loss_vec(
            self.train_state_vec.train_loss_vec, self.train_state_vec.val_loss_vec)
    ...
    def test_single_item(self, i: int, title=None, verbose=False, plot=True):
        ...
        x_np, y_np, pred_np = [GeneralMethods.torch_to_complex_numpy(t) for t in [
            x, y, pred]]
        if verbose:
            print(
                f'x_np.shape={x_np.shape},y_np.shape={y_np.shape},pred_np.shape={pred_np.shape}')
        if plot:
    ...
    def fix_datasets_paths(self, new_dataset_path: str = '../../data/datasets', verbose=True):
        ...
        for ds in [self.train_dataset, self.val_dataset]:
            ...
            ds.data_dir_path = os.path.abspath(
                f'{new_dataset_path}{extension}')
            if verbose:
...
class DoubleTrainer:
    def __init__(self, train_dataset: SeparatedRealImagDataset, val_dataset: SeparatedRealImagDataset,
        ...
        self.trainer_real = Trainer(train_dataset, val_dataset, model_real,
                                    device1, batch_size, l_metric_real, optim_real, params)
        self.trainer_imag = Trainer(train_dataset, val_dataset, model_imag,
                                    device2, batch_size, l_metric_imag, optim_imag, params)
    ...
    def train(self, num_epochs: int, mini_batch_size: int = 500, _tqdm=tqdm):
        ...
        self.train_dataset.set_is_real(
            True), self.val_dataset.set_is_real(True)
        self.trainer_real.train(num_epochs, mini_batch_size, _tqdm)

        print('training imaginary part...')
        self.train_dataset.set_is_real(
            False), self.val_dataset.set_is_real(False)
