...
class MTCNN(nn.Module):
    ...
    def forward(self, img, save_path=None, return_prob=False):
        ...
        # Detect faces
        batch_boxes, batch_probs, batch_points, batch_embeddings = self.detect(img, landmarks=True)
        # Select faces
        # ADAR: could be problem here because i added batch embedding and did not tested this script part
        if not self.keep_all:
            batch_boxes, batch_probs, batch_points, batch_embeddings = self.select_boxes(
                batch_boxes, batch_probs, batch_points, batch_embeddings, img, method=self.selection_method
            )
        # Extract faces
        faces = self.extract(img, batch_boxes, save_path)

        if return_prob:
            return faces, batch_probs, batch_embeddings
        else:
            return faces, batch_embeddings
    ...
    def detect(self, img, landmarks=False):
        ...
        with torch.no_grad():
            batch_boxes, batch_points, batch_embeddings  = detect_face(
        ...
        boxes, probs, points, embeddings = [], [], [], []
        for box, point, embedding in zip(batch_boxes, batch_points, batch_embeddings):
            box = np.array(box)
        ...
        if landmarks:
            return boxes, probs, points, embeddings

        return boxes, probs, embeddings

    def select_boxes(
        self, all_boxes, all_probs, all_points, all_embeddings, imgs, method='probability', threshold=0.9,
        center_weight=2.0
    ):
        ...
        selected_boxes, selected_probs, selected_points, selected_embeddings = [], [], [], []
        for boxes, points, embeddings, probs, img in zip(all_boxes, all_points, all_embeddings, all_probs, imgs):
            ...
            embedding = embeddings[box_order][[0]]

            selected_boxes.append(box)
            selected_probs.append(prob)
            selected_points.append(point)
            selected_embeddings.append(point)
        ...
        if batch_mode:
            ...
            selected_embeddings = selected_embeddings[0]

        return selected_boxes, selected_probs, selected_points, selected_embeddings
